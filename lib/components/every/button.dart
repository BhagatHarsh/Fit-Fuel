...
class NavBarButton extends StatefulWidget {
  final int selected;
  const NavBarButton({required Key key, required this.selected})
      : super(key: key);
...
class NavBarButtonState extends State<NavBarButton> {
  ...
  Widget build(BuildContext context) {
    ...
    final Map<int, VoidCallback> actions = {
      0: () => Navigator.push(
          context, MaterialPageRoute(builder: (context) => const DummyPage())),
      1: () => Navigator.push(
          context, MaterialPageRoute(builder: (context) => const DummyPage())),
      2: () => Navigator.push(
          context, MaterialPageRoute(builder: (context) => const DummyPage())),
    ...
    final String iconPath =
        icons[widget.selected] ?? 'assets/images/svg/bell.svg';
